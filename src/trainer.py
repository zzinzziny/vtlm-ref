...
class Trainer(object):
    ...
    def vlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        self.n_sentences = self.n_sentences + params.batch_size
        self.stats['processed_s'] = self.stats['processed_s'] + lengths.size(0)
        self.stats['processed_w'] = self.stats['processed_w'] + txt_pred_mask.sum().item() + img_pred_mask.sum().item()
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        self.stats['processed_s'] = self.stats['processed_s'] + len2.size(0)
        # self.stats['processed_w'] += (len2 - 1).sum().item()
        self.stats['processed_w'] = self.stats['processed_w'] + (len2 - 1).sum().item()
